...
const MAX_LEYLINE_TITLE_BYTES: usize = 1024;
const MAX_LEYLINE_HYPERLINK_BYTES: usize = 4096;
...
impl<T: Timeout> Processor<T> {
    ...
    /// Return and reset metadata-only parser audit counters.
    pub fn take_audit_delta(&mut self) -> crate::ParseAuditDelta {
        let parser = self.parser.take_audit_delta();
        let ansi = mem::take(&mut self.state.audit);
        crate::ParseAuditDelta {
            unknown_sequences: parser.unknown_sequences.saturating_add(ansi.unknown_sequences),
            rejected_actions: parser.rejected_actions.saturating_add(ansi.rejected_actions),
            truncated_sequences: parser
                .truncated_sequences
                .saturating_add(ansi.truncated_sequences),
        }
    }
    ...
    T: Timeout,
{
    ...
    fn execute(&mut self, byte: u8) {
        match byte {
            ...
            _ => {
                self.state.audit.unknown_sequences =
                    self.state.audit.unknown_sequences.saturating_add(1);
            },
        }
    }
    ...
    fn hook(&mut self, params: &Params, intermediates: &[u8], ignore: bool, action: char) {
        self.state.audit.unknown_sequences = self.state.audit.unknown_sequences.saturating_add(1);
    ...
    fn osc_dispatch(&mut self, params: &[&[u8]], bell_terminated: bool) {
        ...
        fn unhandled(audit: &mut crate::ParseAuditDelta, params: &[&[u8]]) {
        ...
        if params[0] == b"52" {
            self.state.audit.rejected_actions = self.state.audit.rejected_actions.saturating_add(1);
        }
        ...
        match params[0] {
            ...
            b"0" | b"2" => {
                if params.len() >= 2 {
                    let title = params[1..]
                        ...
                        .trim()
                        .to_owned();
                    if title.len() > MAX_LEYLINE_TITLE_BYTES || title.contains('\0') {
            ...
            b"22" if params.len() == 2 => {
                ...
                match CursorIcon::from_str(&shape) {
                    ...
                    Err(_) => unhandled(&mut self.state.audit, params),
                }
            },
            ...
            b"50" => {
                    ...
                    && params[1][0..12] == *b"CursorShape="
                {
                    let shape = match params[1][12] as char {
                        ...
                        _ => return unhandled(&mut self.state.audit, params),
                    };
            ...
            // Set clipboard.
            b"52" => {
                // Leyline v0.1 disables OSC 52 at the parser action boundary.
            },
