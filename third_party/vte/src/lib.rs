...
/// Maximum number of raw bytes retained for one OSC sequence.
pub const MAX_OSC_RAW: usize = 16 * 1024;

/// Metadata-only parser counters. Payload bytes are never retained here.
...
/// [`Perform`]: trait.Perform.html
///
/// Generic over the value for the bounded raw Operating System Command buffer.
#[derive(Default)]
...
impl<const OSC_RAW_BUF_SIZE: usize> Parser<OSC_RAW_BUF_SIZE> {
    ...
    /// Return and reset metadata collected since the previous call.
    pub fn take_audit_delta(&mut self) -> ParseAuditDelta {
        core::mem::take(&mut self.audit)
    }

    /// Current OSC storage usage, exposed for bounded-storage contract tests.
    pub fn osc_storage_len(&self) -> usize {
        self.osc_raw.len()
    }
    ...
    fn advance_osc_string<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        if self.osc_overflowed {
            match byte {
                ...
                0x1B => self.osc_escape_pending = true,
                b'\\' if self.osc_escape_pending => {
                    self.osc_end(performer, byte);
                    self.state = State::Ground;
                },
                _ => self.osc_escape_pending = false,
            }
            return;
        }
        match byte {
    ...
    fn osc_end<P: Perform>(&mut self, performer: &mut P, byte: u8) {
        if self.osc_overflowed {
            self.audit.truncated_sequences = self.audit.truncated_sequences.saturating_add(1);
        } else {
            self.action_osc_put_param();
...
mod tests {
    ...
    fn exceed_max_buffer_size() {
        ...
        assert_eq!(parser.osc_storage_len(), 0);
        assert_eq!(parser.take_audit_delta().truncated_sequences, 1);

        parser.advance(&mut dispatcher, b"recovered");
        assert_eq!(dispatcher.dispatched.len(), 9);
        assert_eq!(dispatcher.dispatched[0], Sequence::Print('r'));
    }

    #[test]
    fn overflow_discards_until_a_complete_string_terminator() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, b"\x1b]2;");
        parser.advance(&mut dispatcher, &[b'a'; MAX_OSC_RAW]);

        // An ESC not followed by '\\' is still untrusted OSC payload after overflow.
        parser.advance(&mut dispatcher, b"\x1bXhidden");
        assert!(dispatcher.dispatched.is_empty());
        assert_eq!(parser.take_audit_delta().truncated_sequences, 0);

        // ST can be split across advance calls and produces one metadata event only.
    ...
    #[test]
    fn osc_at_exact_limit_dispatches() {
        let mut dispatcher = Dispatcher::default();
        let mut parser = Parser::new();
        parser.advance(&mut dispatcher, b"\x1b]");
        parser.advance(&mut dispatcher, &[b'a'; MAX_OSC_RAW]);
        assert_eq!(parser.osc_storage_len(), MAX_OSC_RAW);
        parser.advance(&mut dispatcher, b"\x07");
        assert!(matches!(dispatcher.dispatched.as_slice(), [Sequence::Osc(_, true)]));
        assert_eq!(parser.take_audit_delta().truncated_sequences, 0);
    }
